...
def main():
    ...
    #issueCrawl(driver)
    youngDocCrawl(driver)
    driver.close()


def issueCrawl(driver):
    date = datetime.now().strftime("%Y-%m-%d_%H_%M_%S")
    filename = f"result/mediGate/medigate_IssueList_{date}.txt"
...
def youngDocCrawl(driver):
    date = datetime.now().strftime("%Y-%m-%d_%H_%M_%S")
    filename = f"result/mediGate/medigate_youngDocList_{date}.txt"
    with open(filename, 'w', encoding='utf-8') as fname:
        page_url = []
        ...
        for pageIndex in range(100):
            time.sleep(uniform(0.01, 1.0))
            ##medigate issue list
            url = f'https://www.medicaltimes.com/Main/News/List_Opinion.html?page={pageIndex}&MainCate=7&SubCate=40&TargetDate=&keyword='
            try:
                driver.get(url)
            except:
                break
